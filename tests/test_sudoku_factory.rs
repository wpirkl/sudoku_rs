...
fn test_returns_none_for_empty_mask() {
    ...
    let mut sudoku_factory = SudokuFactory::<9, 9>::new(Box::new(StdRng::seed_from_u64(42)));
    let mask = 0;
    assert_eq!(sudoku_factory.select_random_bit(mask), None);
}
...
fn test_returns_exact_index_for_single_bit(){
    ...
    for i in 0..10 as u32 {
        let mask = 1 << i;
        assert_eq!(sudoku_factory.select_random_bit(mask), Some(i));
    }
}
...
fn test_picks_valid_options_only() {
    ...
    for _ in 0..50 {
        ...
        let result = sudoku_factory.select_random_bit(mask).unwrap();
...
fn test_eventually_picks_all_options() {
    ...
    for _ in 0..100 {
        match sudoku_factory.select_random_bit(mask) {
